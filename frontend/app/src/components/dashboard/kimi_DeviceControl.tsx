...
export default function DeviceControl() {
  ...
  const toggleDevice = (id: string) => {
    const newStatus = !devices.find(d => d.id === id)?.status;
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <Icon className="w-5 h-5" style={{ color: isOn ? device.iconColor : 'rgba(255,255,255,0.25)' }} />
                <div>
              ...
              <button onClick={() => toggleDevice(device.id)} className={`toggle-track ${isOn ? 'on' : ''}`}>
      ...
      <div className="mt-3 pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div className="relative w-full" style={{ height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.08)' }}>
          <div
            ...
            style={{ width: `${((hvacTemp - 16) / (30 - 16)) * 100}%`, borderRadius: 2, background: '#0a9396' }}
          />
          <input
            type="range" min={16} max={30} step={0.5} value={hvacTemp}
          ...
          <div
            ...
            style={{ left: `calc(${((hvacTemp - 16) / (30 - 16)) * 100}% - 8px)`, width: 16, height: 16, borderRadius: '50%', background: '#002a35', border: '2px solid #0a9396' }}
          />
        </div>
