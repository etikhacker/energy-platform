      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        {[
          ...
          <div key={s.label} style={{
            background: 'rgba(0,22,30,0.82)', border: '1px solid rgba(255,255,255,0.1)',
            borderRadius: 12, padding: 20,
          }}>
            <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', margin: '0 0 8px 0' }}>{s.label}</p>
            <p style={{ fontSize: 28, fontWeight: 600, color: s.color, fontFamily: 'JetBrains Mono', margin: 0 }}>{s.value}</p>
          </div>
        ))}
      </div>
          ...
          {filtered.map(m => (
            <div key={m.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                {!m.oxunub && (
                  <button onClick={() => oxunduIsaretle(m.id)} style={{
                    padding: '6px 14px', fontSize: 11, borderRadius: 8,
                    border: '1px solid rgba(255,255,255,0.12)',
                    background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.6)',
                    cursor: 'pointer',
                  }}>Oxundu işarələ</button>
                )}
                <button onClick={() => sil(m.id)} style={{
                  padding: '6px 10px', borderRadius: 8,
                  border: '1px solid rgba(230,57,70,0.2)',
                  background: 'rgba(230,57,70,0.08)', cursor: 'pointer',
